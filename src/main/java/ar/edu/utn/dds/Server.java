...
public class Server {
    ...
    public static Object filtroPorAnio(Request request, Response response) throws Exception {

        automotores = automotores.stream().filter(
                	auto -> (new Integer (auto.getAnio()).toString()).equals(request.params("anio"))
        				).collect(Collectors.toList());
